...
const STATUS_VFX = { burn: 'burn', poison: 'poison', freeze: 'freeze', stun: 'stun', bleed: 'bleed' }
...
export class Phase1_5GameEngine extends Phase1GameEngine {
    ...
    this._update = (dt) => {
      ...
      if (this.__phase15VfxEnabled) {
        ...
        for (const n of newNumbers) {
          this.spawnVfx(n.status || (n.crit ? 'crit' : 'hit'), n.x, n.y, n.status ? 0.35 : 0.28, n.crit ? 0.9 : 0.65)
        }
        ...
        for (const enemy of this.enemies) {
          ...
          for (const key of Object.keys(STATUS_VFX)) {
            ...
            if (nowActive && !before[key]) this.spawnVfx(STATUS_VFX[key], enemy.x, enemy.y, 0.8, 0.9)
          }
        }

        for (const [enemy, before] of beforeEnemies) {
          if (enemy.dead && !before.dead) this.spawnVfx('death', before.x, before.y, 0.7, 1.0)
        }
      }
  ...
  spawnVfx(type, x, y, life = 0.5, scale = 1) {
    this.__phase15Vfx.push({
      type, x, y, life, maxLife: life, scale, rotation: 0,
      rotationSpeed: type === 'freeze' ? 0.8 : 0,
      floatSpeed: type === 'burn' || type === 'poison' ? 2 : 0,
    })
  }

  setVfxEnabled(enabled) { this.__phase15VfxEnabled = !!enabled }
}
