...
export class GameEngine {
  ...
  useSkill(key) {
    ...
    if (p.level < def.unlockLevel) return { ok: false, reason: 'locked' }
    if (p.skillCooldowns[key] > 0) return { ok: false, reason: 'cooldown' }
    if (p.mana < def.manaCost) return { ok: false, reason: 'no-mana' }

    p.mana -= def.manaCost
    p.skillCooldowns[key] = def.cooldown

    if (key === 'nova') {
      const dmg = 15 * p.attackMultiplier
      for (const e of this.enemies) {
        if (dist(e.x, e.y, p.x, p.y) < def.radius + e.radius) {
          e.hp -= dmg
          e.hitFlash = 0.15
          if (e.hp <= 0) {
            e.dead = true
      ...
      this._novaFx = { x: p.x, y: p.y, radius: def.radius, life: 0.3, maxLife: 0.3 }
    } else if (key === 'heal') {
      const healAmount = 40 + p.level * 3
  ...
  _update(dt) {
    ...
    // Mana regen and skill cooldowns
    p.mana = Math.min(p.mana + p.manaRegenRate * dt, p.maxMana)
    for (const key of Object.keys(p.skillCooldowns)) {
      if (p.skillCooldowns[key] > 0) p.skillCooldowns[key] -= dt
    }
    if (this._novaFx) {
      this._novaFx.life -= dt
      if (this._novaFx.life <= 0) this._novaFx = null
    }
